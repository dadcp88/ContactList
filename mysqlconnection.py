...
def contact_list_select_last():  # retreive the last register of the database
    connection = mysql_connection()  # using the function created to make the connection to mysql server
    filename = get_query(
        'lastresultDB.sql')  # you have to specify the query file you wan to use into get_query function
    # filename is the content of the file, the query itself
    cursor = connection.cursor()
    cursor.execute(filename)
    result = cursor.fetchall()
...
def contact_list_insert(name_parameter, email_parameter, address_parameter,
                        phone_number_parameter):  # insert a new contact into the database
    connection = mysql_connection()  # using the function created to make the connection to mysql server
    filename = get_query('insertDB.sql')  # you have to specify the query file you wan to use into get_query function
    prepare_query = filename.format(name_parameter, email_parameter, address_parameter,
                                    phone_number_parameter)  # insert the variables sent by the user into the query
    cursor = connection.cursor()
